...
class MockTransaction implements Transaction {
  ...
  @override
  Future<int> rawUpdate(String sql, [List<Object?>? arguments]) => db.rawUpdate(sql, arguments);
...
void main() {
  group('Completion ID Determinism Tests', () {
    ...
    test('Two consecutive submits for non-interval routine result in exactly 1 completion row', () async {
      const routineId = 'r_deterministic_test';
      const dateStr = '2026-08-04';

      await db.insert('routines', {'id': routineId, 'title': 'Non-interval Routine'});
      ...
      // 2nd submit for same routine and date
      await CompletionGateway.instance.submit(
        const RoutineCompletion(
          routineId: routineId,
          dateStr: dateStr,
      ...
      expect(completions.first['id'], equals('comp_${routineId}_$dateStr'));
    });
  });
}
